...
from hqresize import HQResizeOp
...
class MyVideoProcessingApp(Application):
    ...
    def compose(self):
        ...
        roi = None
        # roi = (30, 80, height-60, width-160)
        image_processing = HQResizeOp(
            self,
            in_tensor_format="HWC",
            shape=shape,
            antialias=True,
        ...
        visualizer = HolovizOp(
            self,
            name="holoviz",
            height=shape[0], #  if roi is None else roi[-1],
            width=shape[1], # if roi is None else roi[-2],
